...
__global__ void bajar_fichas(char* dev_tablero) {
	...
	for (int i = pos; i >= dev_M; i -= dev_M) {
		if (dev_tablero[i * 2] == '0') {
			if (dev_tablero[(i - dev_M) * 2] != '0') {
				dev_tablero[i * 2] = dev_tablero[(i - dev_M) * 2];
				dev_tablero[i * 2 + 1] = dev_tablero[(i - dev_M) * 2 + 1];
				dev_tablero[(i - dev_M) * 2] = '0';
				dev_tablero[(i - dev_M) * 2 + 1] = '0';
			}
		}
	}
}
...
__global__ void eliminar_bomba(char* dev_tablero, int* dev_coordenadas, int* dev_fichaInf, curandState* globalState) {
	int pos = (threadIdx.y * blockDim.x + threadIdx.x) * 2;
...
int main(int argc, const char* argv[]) {
	...
	while (vidas > 0) {
		...
		ficha = (h_coordenadas[1] * M + h_coordenadas[0]) * 2;
