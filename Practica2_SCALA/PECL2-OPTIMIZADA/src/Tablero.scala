...
class Tablero() {
  ...
  //Función recursiva que devuelve la posición de la ficha que se debe tocar para eliminar el mayor número fichas
  def eliminarMasFichas(tablero: List[Int], masEliminadas: Int, mejorPos: Int, pos: Int, col: Int, dif: Int): Int = {
    pos match {
      case -1 => mejorPos                                                                 //Si hemos recorrido el tablero entero, devolvemos la mejor posición que hayamos encontrado
      case _ =>
        val (tabAux, _) = realizarMovimiento(tablero, pos % col, (pos - (pos % col)) / col, col, dif)
        val eliminadas = contarFichasEliminadas(tabAux)                                   //Realizamos el movimiento y contamos el número de fichas que se eliminan
        if (eliminadas > masEliminadas) eliminarMasFichas(tablero, eliminadas, pos, pos - 1, col, dif)
        else eliminarMasFichas(tablero, masEliminadas, mejorPos, pos - 1, col, dif)       //Si las fichas eliminadas son más que las que teníamos guardadas, acyualizamos la posición y hacemos la llamada recursiva con el nuevo dato y la siguiente posición. Si no, simplemente hacemos la llamada recursiva con la siguiente posición
    }
  }

  //Función que devuelve el número de fichas que se eliminarán tocando la posición indicada del tablero
  def ejecutarMovimiento(tablero: List[Int], pos:Int, col: Int): Int = {
    val elem = getElem(pos, tablero)                                                      //Obtenemos el elemento que se encuentra en la posición del tablero
    elem match {
      case _ if elem < 7 =>                                                               //Si el elemento es menor que 7, contamos el número de fichas que se borran al interactuar con esa ficha normal
        contarFichasEliminadas(eliminarFichas(tablero, pos, col, lengthCustom(tablero) / col, elem))
      case 8 =>                                                                           //Si el elemento es 8, contamos el número de fichas que se borran al hacer explotar la bomba
        contarFichasEliminadas(activarBomba(tablero, pos, col, rand.nextInt(2)))
      case 9 =>                                                                           //Si el elemento es 9, contamos el número de fichas que se borran al hacer estallar la TNT
        contarFichasEliminadas(activarTnt(tablero, pos, col))
      case _ =>                                                                           //En cualquier otro caso, contamos el número de fichas que se borran al activar el rompecabezas
        contarFichasEliminadas(insertar(0, pos, activarRompe(tablero, elem % 10)))
    }
  }

  //Función recursiva que busca el valor más alto de todos los elementos de una lista
  def buscarMejorMovimiento(tablero:List[Int], pos:Int, mPos:Int, mValor:Int): Int = {
    tablero match {
      case Nil => mPos                                                                    //Si nos hemos recorrido el tablero entero, devuelve el valor más alto que haya encontrado
      case _ =>
        if (tablero.head > mValor) buscarMejorMovimiento(tablero.tail, pos + 1, pos, tablero.head)
        else buscarMejorMovimiento(tablero.tail, pos + 1, mPos, mValor)                   //Si el valor al inicio del tablero es mayor que el que teníamos guardado, lo cambiamos y hacemos la llamada recursiva con el nuevo valor y el resto del tablero. Si no, simplemente hacemos la llamada recursiva con el resto del tablero
    }
  }
}